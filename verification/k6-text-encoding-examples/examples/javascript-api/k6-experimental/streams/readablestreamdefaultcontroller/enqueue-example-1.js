// Source: docs/sources/k6/next/javascript-api/k6-experimental/streams/readablestreamdefaultcontroller/enqueue.md (JavaScript block 1)
...
async function getNextLine(file, state) {
  while (true) {
    if (state.remaining.includes('\n')) {
      ...
      state.remaining += state.decoder.decode(
        state.buffer.subarray(0, bytesRead),
        {
          stream: true,
        },
      );
