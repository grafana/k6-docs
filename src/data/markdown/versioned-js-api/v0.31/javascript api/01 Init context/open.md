...
> #### Use [SharedArray](/v0.31/javascript-api/k6-data/sharedarray/) for CSV and JSON files
>
> `open()` often consumes a large amount of memory because every VU keeps a separate copy of the file in memory.
> To reduce the memory consumption, we strongly advise the usage of [SharedArray](/v0.31/javascript-api/k6-data/sharedarray/) for CSV, JSON and other files intended for script parametrization.
