...
| close      | Emitted when the connection is closed by the client [Socket.close()](/javascript-api/v0.31/k6-ws/socket/socket-close) or when the server sends the `close` event with code status 1000 (normal closure). |
