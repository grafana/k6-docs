...
| `Params.responseCallback` | [expectedStatuses](/javascript-api/v0-32/k6-http/expectedstatuses-statuses) | sets a [responseCallback](/javascript-api/v0-32/k6-http/setresponsecallback-callback) only for this request. For performance reasons it's better to initialize it once and reference it for each time the same callback will need to be used|
...
Here is another example using [http.batch()](/javascript-api/v0-32/k6-http/batch-requests) with a `Params` argument:
