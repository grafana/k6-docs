...
| distribution (object)         | The equivalent of `someDefaultLabel: { loadZone: "amazon:us:ashburn", percent: 100 }`. | How the traffic should be distributed across existing [Load Zones](../load-zones). The keys are string labels that will be injected as [environment variables](../cloud-environment-variables). |
