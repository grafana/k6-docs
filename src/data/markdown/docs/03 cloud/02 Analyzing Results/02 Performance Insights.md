...
When a k6 script is executed, we generate some [built-in metrics relating to VUs](https://k6.io/docs/using-k6/metrics/#built-in-metrics), such as the current and max VUs for the test and the iteration duration. We also generate non-VU metrics that are specific to the protocol used, such as [HTTP metrics](https://k6.io/docs/using-k6/metrics/#http-specific-built-in-metrics). These non-VU metrics are typically generated by code within the function being executed.

The Empty Iterations alert is raised when we detect that the number of iterations exceeds the number of non-VU metrics. This discrepancy is usually caused by errors in the script that prevent any code from being executed within an iteration.

Some common reasons for this are:
- Having conditional logic in the script where one or more conditions do not trigger as expected.
- Accidentally commenting out parts of the script (such as those that would have sent requests).
- Using [scenarios](https://k6.io/docs/using-k6/scenarios/) that don't specify the correct function using the `exec` option.
- Having an empty `default` function.

We raise this alert because when your test has too many empty iterations, it may not achieve the expected test throughput and may provide inaccurate results. We recommend you troubleshoot your script by running it locally with multiple iterations, using logging or [http debugging](https://k6.io/docs/using-k6/http-debugging/) as necessary.

*Special Notes:*
- You may have a valid reason to use empty iterations. For example, if you are running a test with the purpose of injecting data in preparation for a load test, you may be purposely producing empty iterations depending on the type of data already present in the environment. In this case, you may safely ignore this alert.
