...
We also create a grpc client and load the protobuffer definitions for the [HelloService](https://github.com/moul/pb/blob/master/hello/hello.proto) service.
...
The `disrupt` function creates a `ServiceDisruptor` for the `grpcbin` service in the namespace `httpbin`.
...
The tests requires the deployment of the `grpcbin` application. The application must also be accessible using an external IP available in the `SVC_IP` environment variable.
...
You must set the environment variable `GRPC_HOST` with the external IP address and port used to access the `grpc` service from the test script. 
