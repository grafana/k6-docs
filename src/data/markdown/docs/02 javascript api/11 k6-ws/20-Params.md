...
| `Params.tags`         | object | Key-value [custom metric tags](/using-k6/tags-and-groups/#user-defined-tags), where the keys are names of tags and the values are tag values. [Metrics samples generated from the WebSocket connection](/javascript-api/k6-ws/socket/#websocket-built-in-metrics) will have these tags attached, allowing users to filter the results data or set [thresholds on sub-metrics](/using-k6/thresholds/#thresholds-on-tags). |
