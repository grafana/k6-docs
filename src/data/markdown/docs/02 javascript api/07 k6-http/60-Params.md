...
| `Params.timeout`      | string | Maximum time to wait for the request to complete. Default timeout is 60 seconds (`"60s"`). Note that for backwards compatibility a number can also be used, in which case it will be interpreted as milliseconds, e.g. `60000` is equivalent to `"60s"`. |
