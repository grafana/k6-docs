...
| [page.locator(selector\[, options\])](https://k6.io/docs/javascript-api/xk6-browser/api/locator) | - | Creates and returns a new page `locator` given a selector with strict mode on. The strict mode only allows selecting a single matching element, and will throw an error if multiple matches are found. |
