...
By default, `__ENV` is a plain, mutable object: assignments like `__ENV.MY_HOSTNAME = 'other'` persist for the rest of the iteration. To make `__ENV` read-only instead — so modifying it throws a `TypeError` in strict-mode code, or fails silently otherwise — enable the `freeze-env` [feature flag](https://grafana.com/docs/k6/<K6_VERSION>/using-k6/feature-flags/).
