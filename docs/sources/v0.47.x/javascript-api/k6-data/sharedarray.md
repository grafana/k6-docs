...
You must construct a `SharedArray` in the [`init` context](/docs/k6/<K6_VERSION>/using-k6/test-lifecycle).
...
Attempting to instantiate a `SharedArray` outside of the [init context](/docs/k6/<K6_VERSION>/using-k6/test-lifecycle) results in the exception `new SharedArray must be called in the init context`.
