...
The following script uses the [k6 HTTP module](/docs/k6/<K6_VERSION>/javascript-api/k6-http/) to test a single endpoint.
...
- [Smoke test](/docs/k6/<K6_VERSION>/testing-guides/test-types/smoke-testing). Verify the system functions with minimal load.
- [“Average” load test](/docs/k6/<K6_VERSION>/testing-guides/test-types/load-testing). Discover how the system functions with typical traffic.
- [Stress test](/docs/k6/<K6_VERSION>/testing-guides/test-types/stress-testing). Discover how the system functions with the load of peak traffic.
- [Spike test](/docs/k6/<K6_VERSION>/testing-guides/test-types/spike-testing). Discover how the system functions with sudden and massive increases in traffic.
- [Breakpoint test](/docs/k6/<K6_VERSION>/testing-guides/test-types/breakpoint-testing). Progressively ramp traffic to discover system breaking points.
- [Soak test](/docs/k6/<K6_VERSION>/testing-guides/test-types/soak-testing). Discover whether or when the system degrades under loads of longer duration.
...
To configure the workload, use [test options](/docs/k6/<K6_VERSION>/using-k6/k6-options/).
...
Generally, your load tests should add [sleep time](/docs/k6/<K6_VERSION>/javascript-api/k6/sleep).
...
To configure workloads according to a target request rate, use the [constant arrival rate executor](/docs/k6/<K6_VERSION>/using-k6/scenarios/executors/constant-arrival-rate).
...
To ramp the request rate up or down, use the [`ramping-arrival-rate`](/docs/k6/<K6_VERSION>/using-k6/scenarios/executors/ramping-arrival-rate) executor instead.

For all ways to model the load in k6, refer to [Scenarios](/docs/k6/<K6_VERSION>/using-k6/scenarios).
...
[Checks](/docs/k6/<K6_VERSION>/using-k6/checks) validate conditions during the test execution.
...
In k6, you can use [Thresholds](/docs/k6/<K6_VERSION>/using-k6/thresholds) to set the test pass/fail criteria.
...
You can parameterize the users with the [`SharedArray`](/docs/k6/<K6_VERSION>/javascript-api/k6-data/sharedarray) object as follows:
...
To read more about data parameterization, check out the [parameterization examples](/docs/k6/<K6_VERSION>/examples/data-parameterization) and [Execution context variables](/docs/k6/<K6_VERSION>/using-k6/execution-context-variables).
...
- Run the test from your private network using the k6 run command or the [Kubernetes operator](https://github.com/grafana/k6-operator). Optionally, store the test results in [k6 Cloud](/docs/k6/<K6_VERSION>/results-output/real-time/cloud) or other [external services](/docs/k6/<K6_VERSION>/results-output/real-time/).
- For cloud tests:
...
Even so, we recommend you get familiar with the [k6 Javascript API](/docs/k6/<K6_VERSION>/javascript-api) and script your own tests.
...
- [HTTP/1.1, HTTP/2](/docs/k6/<K6_VERSION>/javascript-api/k6-http/)
- [WebSockets](/docs/k6/<K6_VERSION>/javascript-api/k6-ws/)
- [Redis (experimental)](/docs/k6/<K6_VERSION>/javascript-api/k6-experimental/redis/)
- [gRPC](/docs/k6/<K6_VERSION>/javascript-api/k6-net-grpc/)
...
If k6 doesn't support a protocol you need, you can use (or create) [extensions](/docs/k6/<K6_VERSION>/extensions).
...
- MLLP
- NATS
- and [more](/docs/k6/<K6_VERSION>/extensions/explore).
