...
- [**Browser test editor**](https://grafana.com/docs/k6-studio/components/browser-editor/): The browser test editor lets you build and debug a k6 browser test using a visual interface. You can add actions and assertions that interact with elements on a page, and export the test as a k6 test script.
- [**Debugger**](https://grafana.com/docs/k6-studio/components/debugger/): The Debugger can help you validate that a test script is working as expected. You can use it to run one iteration of your test, and visualize the request and response of any requests on your test script. The Debugger works with any k6 test script, not only scripts generated via the Generator.

Each component can be used separately. If you have a HAR recording, you can add it to the Recorder folder to visualize the request and response data, or import it into the Generator to create a new test script. You can also import a script you create manually to the Debugger, and use it to run one iteration of that test.
