...
{{< figure src="/media/docs/k6-studio/screenshot-k6-studio-2.0-homepage.png" alt="k6 Studio homepage" >}}
...
The k6 Studio desktop application consists of four components:
...
- **Browser test editor**: The browser test editor lets you build and debug a k6 browser test using a visual interface. You can add actions and assertions that interact with elements on a page, and export the test as a k6 test script.
- **Debugger**: The Debugger can help you validate that a test script is working as expected. You can use it to run one iteration of your test, and visualize the request and response of any requests on your test script. The Debugger works with any k6 test script, not only scripts generated via the Generator.
