...
If we inspect the data of the **POST** request we can see that it makes use of a `csrf` token.
...
This is it, with this rule in place we have added dynamic data correlation to our script, without having to touch any code!

## Insert a custom javascript snippet

We can make use of the **Custom Code** rule to insert a custom javascript snippet in our script.

- In k6 Studio, press **+ Add rule** and select **Custom Code** from the entries.
- In the **snippet** input write `console.log('hello k6')`

If you have the script pane open you will see the scrip getting updated in real-time with your snippet inserted.

## Validate a test script

To validate a test script we can press the **Validate** button in the script preview pane.

The validator will open running your script in a single iteration and allowing you to inspect the requests and responses sent plus additional details like logs coming from k6 and checks. (The checks are the ones generated by the default **Verification rule** that gives us a quick glimpse if the script is working as expected)

In the validator it it possible to inspect requests details as before, giving you a powerful tool to debug your script.

## Export a test script

To export a test script we can use the **Export** button in the script pane of the generator.

It will ask us for a script name and we can export it.

We can right click the script name to open the folder containing the script.

Now you can use `k6` to run the script.
